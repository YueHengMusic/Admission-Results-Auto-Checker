...
通过 `queryWindowEnabled` 开关和 `queryStartHour`/`queryEndHour` 配置，限制程序只在指定时间段内运行。支持正常区间（8:00~17:30）和跨夜区间（22:00~6:00），时间格式支持 `"8:30"` 字符串、`8.5` 小数、`8` 整数。到点后完成当次查询再停止，不截断正在进行的查询。
